...
public class Words
{
     ...
     *
     * @param word
     *         Часть слова для поиска
     * @param handler
     *         Обработчик, в который прийдет набор слов в случае успешного выполнения. Тип сообщения
     *         - {@link #MSG_GET_WORDS}
     */
    public void getWordsSync(
            String word,
            Handler handler)
    {
    ...
    Comparator<WordEntry> m_wordsComparator = new Comparator<WordEntry>()
    {
        ...
        public int compare(
                ...
                WordEntry object2)
        {
            if (object1.compareType > object2.compareType) {
                return 1;
            }
            if (object1.compareType < object2.compareType) {
                return -1;
            }
            if (object1.freq < object2.freq) {
                return 1;
            }
            if (object1.freq > object2.freq) {
                return -1;
     ...
     * @param cs
     *         Изначальный регистр слова
     *
     * @return Возвращает обработанный массив
     */
    Vector<WordEntry> postProcessWords(
            Vector<WordEntry> ar,
            int cs)
    {
