...
public class TextTools
{
    ...
    /**
     * Значение compare - текст в исходной строке отличается от буфера не более, чем на CORRECT_SIZE
     * букв
     */
    ...
    public static final String changeCase(
            String text,
            int cs)
    {
        if (text.length() == 0) {
     ...
     * @param c2
     *         Второй символ, в любом регистре
     *
     * @return true, если символы совпадают, иначе false
     */
    public static final boolean isCharEqual(
            char c1,
            char c2)
    {
     ...
     *
     * @param str
     *         Исходная строка для сравнения в нижнем регистре
     * @param cb
     *         Буфер для сравнения из словаря, после слова идет пробельный символ
     *
            ...
            CharBuffer cb)
    {
        ...
        int corr = 0;
        int wordLen =
                0; // Длина слова, неполная, если строка короче (но по крайней мере, больше на 1)
        ...
        if (ds == 0) {
            return corr == 0 ? COMPARE_TYPE_EQUAL : COMPARE_TYPE_CORRECTION;
                   ...
                   ds < 0)// исходное слово длинее, чем в словаре, хотя все буквы совпали
        {
            if (Math.abs(ds) > CORRECT_SIZE) {
                return COMPARE_TYPE_NONE;
            }
            ...
            String str1)
    {
        ...
        if (len - len1 <= CORRECT_SIZE && corr <= CORRECT_SIZE) {
            return COMPARE_TYPE_CORRECTION;
        }
        return COMPARE_TYPE_NONE;
    }
