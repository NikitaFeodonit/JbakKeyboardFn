...
public class KeyboardPaints
{
    ...
    /**
     * Цвета текстов. 0 - основной текст, 1 - текст доп символов, 2 - основной текст в нажатом
     * состоянии, 3 - доп. символы в нажатом состоянии
     */
...
//                design.m_keyBackground.setDependentback(design.m_kbdFuncKeys.m_keyBackground);
                ((CustomButtonDrawable) st.kv().m_KeyBackDrw).setDependentDrawable(
                        funcBackDrawable);
            }
        } else {
    ...
    public static final int getScreen(
            Context c,
            boolean bPortrait)
    {
     ...
     *         Контекст
     * @param bPortrait
     *         - true - для расчёта используется большее значение ширины/высоты, false - меньшее
     * @param val
     *         Значение для перевода
     *
     * @return
     */
    public static final float pixelToPerc(
            Context c,
            boolean bPortrait,
            float val)
    {
     ...
     *         Контекст
     * @param val
     *         Значение а процентах от высоты экрана
     * @param bPortrait
     *         - true - для расчёта используется большее значение ширины/высоты, false - меньшее
     * @param bEven
     *         true - вернуть четное значение
     *
     * @return Размер в пикселях
     */
    public static int percToPixel(
            Context c,
            boolean bPortrait,
            float val,
            boolean bEven)
            ...
            int type)
    {
        switch (type) {
            case VAL_KEY_HEIGHT_PORTRAIT:
                return percToPixel(
                        c, true, p.getFloat(st.PREF_KEY_HEIGHT_PORTRAIT_PERC, getDefValue(type)),
                        true);
            case VAL_KEY_HEIGHT_LANDSCAPE:
                return percToPixel(
                        c, false, p.getFloat(st.PREF_KEY_HEIGHT_LANDSCAPE_PERC, getDefValue(type)),
                        true);
            case VAL_TEXT_SIZE_MAIN:
            case VAL_TEXT_SIZE_SYMBOL:
            case VAL_TEXT_SIZE_LABEL:
