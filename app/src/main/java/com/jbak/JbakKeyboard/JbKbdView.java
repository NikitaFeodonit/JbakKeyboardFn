        ...
        extends KeyboardView
{
    ...
    /**
     * Состояние - клавиши в верхнем регистре на одну букву. После ввода любого символа -
     * сбрасывается
     */
    ...
    void init()
    {
        ...
        String path = st.pref()
                .getString(st.PREF_KEY_KBD_SKIN_PATH, st.NULL_STRING + st.KBD_DESIGN_STANDARD);
...
//        boolean bp = pref.getBoolean(st.PREF_KEY_PREVIEW, true);
        ...
        m_KeyHeight = KeyboardPaints.getValue(
                getContext(), pref, bPortrait
                                    ? KeyboardPaints.VAL_KEY_HEIGHT_PORTRAIT
                                    : KeyboardPaints.VAL_KEY_HEIGHT_LANDSCAPE);
        m_vertCorr = pref.getInt(
                st.isLandscape(getContext())
                ? st.PREF_KEY_CORR_LANDSCAPE
                : st.PREF_KEY_CORR_PORTRAIT, defaultVertCorr);
    }
    ...
    public void setLang(String newLang)
    {
        ...
        if (k == null) {
            Toast.makeText(getContext(), "No keyboards for lang " + newLang, Toast.LENGTH_LONG)
                    .show();
            return;
        }
     ...
     *
     * @param canUseMenu
     *         Если false - меню выбора языков не выводится
     * @param nextLang
     *         следующий язык, false - предыдущий
     */
    ...
    public void gesture(GestureInfo gest)
    {
        ...
        if (m_handler != null) {
            ...
            m_handler.sendMessage(
                    m_handler.obtainMessage(OwnKeyboardHandler.MSG_REMOVE_PREVIEW, null));
        }
//        resetPressed();
        gestureProcessed = true;
        //       invalidateAllKeys();
        KbdGesture g = null;
        if (gest.downKey != null && gest.downKey.codes[0] == 32 &&
            (gest.dir == GestureInfo.LEFT || gest.dir == GestureInfo.RIGHT)) {
