        ...
        implements OnSharedPreferenceChangeListener
{
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        arIntEntries = new IntEntry[] {
                new IntEntry(
                        st.PREF_KEY_USE_SHORT_VIBRO, R.string.set_key_short_vibro_desc,
                        R.array.vibro_short_type, st.ONE_STRING), new IntEntry(
                st.PREF_KEY_AC_PLACE, R.string.set_key_ac_place_desc, R.array.ac_place,
                st.ZERO_STRING), new IntEntry(
                st.PREF_KEY_PORTRAIT_TYPE, R.string.set_key_portrait_input_type_desc,
                R.array.array_input_type, st.ZERO_STRING), new IntEntry(
                st.PREF_KEY_LANSCAPE_TYPE, R.string.set_key_landscape_input_type_desc,
                R.array.array_input_type, st.ZERO_STRING), new IntEntry(
                st.PREF_KEY_PREVIEW_TYPE, R.string.set_ch_keys_preview_desc, R.array.pv_place,
                st.ONE_STRING), new IntEntry(
                st.PREF_KEY_USE_VOLUME_KEYS, R.string.set_key_use_volumeKeys_desc, R.array.vk_use,
                st.ZERO_STRING), new IntEntry(
                st.PREF_KEY_SOUND_VOLUME, R.string.set_key_sounds_volume_desc, R.array.integer_vals,
                "5"),};
        ...
        pr.setSummary(pr.getSummary().toString() + '\n' + getBackupPath());
        setSummary(
                st.PREF_KEY_CLIPBRD_SIZE, R.string.set_key_clipbrd_size_desc,
                p.getString(st.PREF_KEY_CLIPBRD_SIZE, DEF_SIZE_CLIPBRD));
        ...
        for (IntEntry ie : arIntEntries) {
            ...
            setSummary(
                    ie.key, ie.descStringId,
                    strVal(getResources().getStringArray(ie.arrayNames)[index]));
        }
    ...
    void showHelper()
    {
        ...
        int step = 0;
        String curId = Settings.Secure.getString(
...
// Проверяем регистрацию в InputMethodManager        
        ...
        if (step == 0) {
            ...
            pr.setSummary(
                    getString(R.string.helper_1_desc) + " \"" + getString(R.string.ime_name) +
                    "\"");
        } else if (step == 1) {
    ...
    void runSetKbd(int action)
    {
        try {
            Intent in = new Intent(Intent.ACTION_VIEW).setComponent(
                    new ComponentName(this, SetKbdActivity.class))
            ...
            Preference preference)
    {
        ...
        if ("intervals".equals(k)) {
            ...
            c.startActivity(
                    new Intent(c, EditSetActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                            .putExtra(EditSetActivity.EXTRA_PREF_KEY, st.PREF_KEY_MAIN_FONT)
                            .putExtra(
                                    EditSetActivity.EXTRA_DEFAULT_EDIT_SET,
                                    st.paint().getDefaultMain().toString()));
            ...
            c.startActivity(
                    new Intent(c, EditSetActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                            ...
                            .putExtra(
                                    EditSetActivity.EXTRA_DEFAULT_EDIT_SET,
                                    st.paint().getDefaultSecond().toString()));
            ...
            c.startActivity(
                    new Intent(c, EditSetActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                            ...
                            .putExtra(
                                    EditSetActivity.EXTRA_DEFAULT_EDIT_SET,
                                    st.paint().getDefaultLabel().toString()));
            ...
            getApplicationContext().startActivity(
                    new Intent(getApplicationContext(), EditSetActivity.class).addFlags(
            ...
            getApplicationContext().startActivity(
                    new Intent(getApplicationContext(), EditSetActivity.class).addFlags(
                            Intent.FLAG_ACTIVITY_NEW_TASK)
                            .putExtra(
                                    EditSetActivity.EXTRA_PREF_KEY,
                                    st.PREF_KEY_FONT_PANEL_AUTOCOMPLETE)
                            .putExtra(
                                    EditSetActivity.EXTRA_DEFAULT_EDIT_SET,
                                    JbCandView.getDefaultEditSet(this).toString()));
            return true;
    ...
    void setShiftState()
    {
        ...
        setSummary(
                st.PREF_KEY_SHIFT_STATE, 0,
                getResources().getStringArray(R.array.array_shift_vars)[v]);
    }
            ...
            String key)
    {
        if (st.PREF_KEY_GESTURE_LEFT.equals(key) || st.PREF_KEY_GESTURE_RIGHT.equals(key) ||
            ...
            st.PREF_KEY_GESTURE_SPACE_RIGHT.equals(key)) {
            JbKbdView.inst = null;
    ...
    void backup(final boolean bSave)
    {
        Dlg.yesNoDialog(
                this,
                getString(bSave ? R.string.set_key_save_pref : R.string.set_key_load_pref) + " ?",
                new st.UniObserver()
                {
                    @Override
                    public int OnObserver(
                        ...
                        if (((Integer) param1).intValue() == AlertDialog.BUTTON_POSITIVE) {
                            ...
                            try {
                                if (ret == 0) {
                                    ...
                                    Toast.makeText(getApplicationContext(), R.string.ok, 700)
                                            .show();
                                }
                                if (!bSave) {
                                    finish();
