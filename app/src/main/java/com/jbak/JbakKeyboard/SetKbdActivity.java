        ...
        extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        if (m_curAction == st.SET_KEY_CALIBRATE_PORTRAIT ||
            m_curAction == st.SET_KEY_CALIBRATE_LANDSCAPE) {
            initCalibrate();
            return;
        }
...
//        m_MainView.setBackgroundDrawable(st.getBack());
        ...
        if (m_curAction == st.SET_KEY_HEIGHT_LANDSCAPE) {
            ...
            String path = st.pref().getString(
                    st.PREF_KEY_KBD_SKIN_PATH, st.NULL_STRING + st.KBD_DESIGN_STANDARD);
            int pos = 0;
            ...
            m_MainView.findViewById(R.id.save).setOnClickListener(
                    new OnClickListener()
                    {
                        @Override
            ...
            m_MainView.findViewById(R.id.screen_type).setOnClickListener(
                    new OnClickListener()
                    {
                        @Override
                        public void onClick(View v)
        ...
        if (m_curAction == st.SET_KEY_HEIGHT_PORTRAIT ||
            m_curAction == st.SET_KEY_HEIGHT_LANDSCAPE) {
            final IntEditor sb = (IntEditor) m_MainView.findViewById(R.id.key_height);
            sb.setSteps(new int[] {2, 4, 8});
            sb.setMinAndMax(20, 200);
            int val = KeyboardPaints.getValue(
                    this, pref, m_curAction == st.SET_KEY_HEIGHT_PORTRAIT
                                ? KeyboardPaints.VAL_KEY_HEIGHT_PORTRAIT
                                : KeyboardPaints.VAL_KEY_HEIGHT_LANDSCAPE);
            sb.setOnChangeValue(
                    new IntEditor.OnChangeValue()
                    {
                        @Override
            ...
            sb.setValue(val);
            ((Button) m_MainView.findViewById(R.id.default_size)).setOnClickListener(
                    new OnClickListener()
                    {
                        @Override
                        public void onClick(View v)
                        {
                            float def = KeyboardPaints.getDefValue(
                                    m_curAction == st.SET_KEY_HEIGHT_PORTRAIT
                                    ? KeyboardPaints.VAL_KEY_HEIGHT_PORTRAIT
                                    : KeyboardPaints.VAL_KEY_HEIGHT_LANDSCAPE);
    ...
    void showScreenTypes()
    {
        ...
        ArrayAdapter<String> adapt = new ArrayAdapter<String>(
                inst, android.R.layout.select_dialog_item,
    ...
    {
        m_screenType = sel;
        ((TextView) m_MainView.findViewById(R.id.screen_type)).setText(
                getResources().getStringArray(R.array.screen_type_vars)[sel]);
    }
    ...
    void saveKeyboard()
    {
        if (m_curAction == st.SET_SELECT_KEYBOARD) {
            ...
            if (m_screenType == 0 || m_screenType == 1) {
                st.pref()
                        .edit()
                        .putString(st.PREF_KEY_LANG_KBD_PORTRAIT + m_LangName, path)
                        .commit();
            }
            if (m_screenType == 0 || m_screenType == 2) {
                st.pref()
                        .edit()
                        .putString(st.PREF_KEY_LANG_KBD_LANDSCAPE + m_LangName, path)
                        .commit();
            }
    ...
    public void onBackPressed()
    {
        if (m_curAction == st.SET_SELECT_SKIN) {
            st.pref().edit().putString(
                    st.PREF_KEY_KBD_SKIN_PATH, st.getSkinPath(st.arDesign[m_curSkin])).commit();
        }
    ...
    void setCalibrateListeners()
    {
        ...
        String calibrSet = m_curAction == st.SET_KEY_CALIBRATE_PORTRAIT
                           ? st.PREF_KEY_CORR_PORTRAIT
        ...
        m_calibrManual.setOnChangeValue(
                new OnChangeValue()
                {
    ...
    void initCalibrate()
    {
        setRequestedOrientation(
                m_curAction == st.SET_KEY_CALIBRATE_PORTRAIT
...
//        m_MainView.setBackgroundDrawable(st.getBack());
        ...
        m_calibrToggle.setOnClickListener(
                new OnClickListener()
                {
                    @Override
                    public void onClick(View v)
            ...
            final int value)
    {
        ...
        String alert =
                String.format(getString(R.string.calibr_save), value, JbKbdView.defaultVertCorr);
        Dlg.yesNoDialog(
                inst, alert, new st.UniObserver()
                {
