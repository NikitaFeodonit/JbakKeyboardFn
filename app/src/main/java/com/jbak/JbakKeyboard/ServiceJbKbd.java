    ...
    void forceFullScreen(EditorInfo attribute)
    {
        ...
        if (set == st.PREF_VAL_EDIT_TYPE_FULLSCREEN) {
            ...
            attribute.imeOptions =
                    st.rem(attribute.imeOptions, 0x02000000/*EditorInfo.IME_FLAG_NO_FULLSCREEN*/);
        }
    }
...
//        Log.d(PressArray.TAG, "sendKey "+ms);
    ...
    /**
     * Предлагает юзеру набор автодополнений. В браузере - показывает какой-то набор из закладок и
     * посещенных ссылок
     */
    @Override
    ...
    public final KeyEvent generateHardwareEvent(
            int action,
            int code,
            int meta)
    {
        return new KeyEvent(
                SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), action, code, 0, meta, 0, 0,
    ...
    public final void sendHardwareSequence(
            InputConnection ic,
            Integer... vals)
    {
        int meta = 0;
        ...
        if (st.has(st.kv().m_state, JbKbdView.STATE_SOUNDS)) {
            ...
            switch (primaryCode) {
                case KeyEvent.KEYCODE_DPAD_LEFT:
                    sound = AudioManager.FX_FOCUS_NAVIGATION_LEFT;
                    break;
                case KeyEvent.KEYCODE_DPAD_RIGHT:
                    sound = AudioManager.FX_FOCUS_NAVIGATION_RIGHT;
                    break;
                case KeyEvent.KEYCODE_DPAD_UP:
                    sound = AudioManager.FX_FOCUS_NAVIGATION_UP;
                    break;
                case KeyEvent.KEYCODE_DPAD_DOWN:
                    sound = AudioManager.FX_FOCUS_NAVIGATION_DOWN;
                    break;
        ...
        if (primaryCode < -200 && primaryCode > -300) {
            ...
            if (k != null) {
                onText(k.getMainText());
                   ...
                   primaryCode >= KeyEvent.KEYCODE_DPAD_UP &&
...
//        }
    ...
    /**
     * Проверяет, нужно ли выполнить переход к qwerty-клавиатуре по нажатию клавиши. Если нужно -
     * выполняет переход
     *
     * @param primaryCode
     *         код нажатой клавиши
    ...
    /**
     * Форсированно показывает окно клавиатуры, запоминает статус в {@link #m_bForceShow}
     */
    void forceShow()
    {
        showWindow(true);
    ...
    /**
     * Возвращает true, если находимся в режиме выделения текста, иначе возвращает false
     */
...
//                    ic.sendKeyEvent(new KeyEvent(KeyEvent.ACTION_UP, KeyEvent.KEYCODE_SHIFT_LEFT));
            ...
            String key)
    {
        ...
        if (st.PREF_KEY_USE_VOLUME_KEYS.equals(key) || key == null) {
            m_volumeKeys = Integer.valueOf(
                    sharedPreferences.getString(st.PREF_KEY_USE_VOLUME_KEYS, st.ZERO_STRING));
        }
        if (st.PREF_KEY_AC_PLACE.equals(key) || key == null) {
            m_acPlace = Integer.valueOf(
                    sharedPreferences.getString(st.PREF_KEY_AC_PLACE, st.ONE_STRING));
        }
        ...
        m_soundVolume /= 10f;
        m_LandscapeEditType =
                Integer.valueOf(sharedPreferences.getString(st.PREF_KEY_LANSCAPE_TYPE, "0"));
        m_PortraitEditType =
                Integer.valueOf(sharedPreferences.getString(st.PREF_KEY_PORTRAIT_TYPE, "0"));
    ...
    final int getCase()
    {
        ...
        try {
            ...
            if (m_textBeforeCursor == null) {
                m_textBeforeCursor =
                        new StringBuffer(getCurrentInputConnection().getTextBeforeCursor(40, 0));
            }
            if (m_textBeforeCursor == null ||
                m_textBeforeCursor.length() == 0 && st.has(m_state, STATE_EMPTY_UP)) {
                return 1;
            }
