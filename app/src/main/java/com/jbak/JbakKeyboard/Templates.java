...
public class Templates
{
    ...
    /**
     * Если m_editFile!=null - переименовывает эту папку в name.<br> Иначе создаёт новую папку с
     * именем name
     */
...
//            os.write(new byte[]{(byte)0xef,(byte)0xbb,(byte)0xbf});
     ...
     *
     * @param f
     *         Файл для чтения, текст должен быть в кодировке UTF-8
     *
     ...
     *
     * @param f1
     *         Позиция символа \r . для bLast=true вычисляется через lastIndexOf, для bLast =false -
     *         через indexOf
     * @param f2
     *         Позиция символа \n. Или наоборот, пофигу
     * @param bLast
     ...
     * @param len
     *         Текущая длина строки. Если len<4000 и ни одного переноса строки не найдено - для
     *         bLast = true вернет 0, для bLast=false - len
     *
     * @return Возвращает позицию конца или начала строки или -1, если не найдено (текст >=4000
     * символов и в нём нет ни одного переноса)
     */
    static int chkPos(
            int f1,
            int f2,
    ...
    /**
     * Класс для получения информации о текущем выделении, текущем слове и текущей строке А также
     * для операций с ними
     */
     ...
     *
     * @param seq
     *         Текст, взятый функцией {@link InputConnection#getTextBeforeCursor(int, int)}. Может
     *         быть null
     *
     * @return Текст начала слова под курсором *
     */
    static String getCurWordStart(
            CharSequence seq,
            boolean bRetEmptyIfNotDelimiter)
    {
        if (seq == null) {
        ...
        for (int i = seq.length() - 1; i >= 0; i--) {
            ...
            if (!Character.isLetterOrDigit(ch)) {
                return seq.subSequence(
                        apostr > -1 && i == apostr - 1 ? apostr + 1 : i + 1, seq.length())
                        .toString();
            }
        }
     ...
     *         - true - если не найден конец слова, вернёт пустую строку. false - вернёт строку seq
     *
     * @return Текст конца слова под курсором *
     */
    static String getCurWordEnd(
            CharSequence seq,
            boolean bRetEmptyIfNotDelimiter)
    {
        if (seq == null) {
