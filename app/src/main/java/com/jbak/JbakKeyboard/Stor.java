        ...
        extends SQLiteOpenHelper
{
    ...
    Stor(Context context)
    {
        ...
        try {
            CLIPBOARD_LIMIT =
                    Integer.decode(st.pref(context).getString(st.PREF_KEY_CLIPBRD_SIZE, "20"));
        } catch (Throwable e) {
            CLIPBOARD_LIMIT = 20;
        }
        inst = this;
        ...
        try {
            m_db.execSQL(CLIPBOARD_TABLE_CREATE);
        } catch (Exception e) {
            st.logEx(e);
        }
        try {
            m_db.execSQL(KEYS_TABLE_CREATE);
        } catch (Exception e) {
            st.logEx(e);
        }
    }
    ...
    void saveClipboardString(String s)
    {
        ...
        try {
            SQLiteStatement stat =
                    m_db.compileStatement("INSERT INTO " + TABLE_CLIPBOARD + " VALUES(?,?,?)");
