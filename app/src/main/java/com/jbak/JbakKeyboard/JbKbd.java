        ...
        extends Keyboard
{
     ...
     *         Ресурсы программы
     * @param options
     *         Тип редактирования, {@link EditorInfo#imeOptions}
     */
    void setImeOptions(
            Resources res,
            int options)
    ...
    /**
     * Собственный класс клавиш. Отнаследован от системного. <br> При создании клавиши, если метка
...
//            }
        ...
        final boolean processUserKeyboard(boolean longPress)
        {
            ...
            try {
                if (t.startsWith("$$")) {
                    ...
                    kb = new Keybrd(
                            IKeyboard.KBD_SYM, st.getLangByName(IKeyboard.LANG_SYM_KBD),
                            R.xml.kbd_empty, R.string.kbd_name_sym_edit);
                    kb.path = t;
                }
        ...
        {
            return longCode != 0 || getUpText() != null || codes[0] == 10 ||
                   codes[0] == Keyboard.KEYCODE_SHIFT || st.has(flags, FLAG_USER_KEYBOARD_LONG) ||
                   st.has(flags, FLAG_USER_TEMPLATE_LONG);
        }
