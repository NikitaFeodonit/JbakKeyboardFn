        ...
        extends RectShape
{
    ...
    /**
     * Фон обводки. Если не null - рисуется под основным фоном, и в этом случае будет рисоваться его
     * тень
     */
    ...
    public static ColorFilter m_pressFilter =
            new PorterDuffColorFilter(0xff888888, PorterDuff.Mode.MULTIPLY);
     ...
     *         Начальный цвет градиента
     * @param endColor
     *         Конечный цвет градиента. Если равен {@link #DEFAULT_COLOR} - то фон заполняется
     *         цветом startColor
     */
    public GradBack set(
            int startColor,
     ...
     *
     * @param cx
     *         Радиус скругления по оси X
     * @param cy
     *         Радиус скругления по оси Y
     *
     * @return Возвращает текущий объект
     */
    public GradBack setCorners(
            int cx,
            int cy)
    {
        m_cornerX = cx;
            ...
            float height)
    {
        ...
        if (m_ptFill != null && width == m_rect.width() + m_gap * 2 &&
            height == m_rect.height() + m_gap * 2) {
            return;
        }
...
//			canvas.drawRoundRect(m_rect, m_cornerX, m_cornerY, m_ptFillPressed);
    ...
    /**
     * Функция отрисовки метки для состояний checked и checkable. Вызывается только при наличии этих
     * состояний
     *
     * @param canvas
     *         Canvas для отрисовки
     * @param bCheck
     *         true - помечено, false - нет
     * @param rect
     *         Прямоугольник, на котором производится отрисовка
     */
    public void onDrawCheckMark(
            Canvas canvas,
            boolean bCheck,
            RectF rect)
    {
    ...
    /**
     * Устанавливает обводку stroke, в виде еще одного объекта {@link GradBack} Необходимо правильно
     * задать отступ (gap) для нового объекта, с учетом того, что сверху будет нарисован текущий
     * объект
     *
     * @param stroke
     *         Объект для рисования обводки
     *
     ...
     *
     * @param s
     *         Проверяемый статус, одна из констант <b>android.R.attr.state_</b>
     *
